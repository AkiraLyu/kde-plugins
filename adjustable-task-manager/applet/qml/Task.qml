...
import plasma.applet.org.kde.plasma.adjustabletaskmanager as TaskManagerApplet
...
PlasmaCore.ToolTipArea {
    ...
    implicitHeight: inPopup
                    ...
                    : (tasksRoot.vertical
                        ? TaskManagerApplet.LayoutMetrics.preferredMinHeight() + launcherSpacing
    ...
    Layout.maximumWidth: {
        ...
        if (model.IsLauncher && !tasksRoot.iconsOnly) {
            return tasksRoot.height / taskList.rows + launcherSpacing;
        }

        const maximum = TaskManagerApplet.LayoutMetrics.preferredMaxWidth();
        if (inPopup || tasksRoot.iconsOnly || Plasmoid.configuration.taskMaxWidth !== 3
            || !Plasmoid.configuration.shrinkTasksToTitle) {
            return maximum;
        }

        const textWidth = Math.max(titleMetrics.width, titleMetrics.advanceWidth,
            TaskManagerApplet.LayoutMetrics.spaceRequiredToShowText());
        const audioWidth = audioStreamIcon?.visible
            ? audioStreamIcon.width + TaskManagerApplet.LayoutMetrics.labelMargin : 0;
        // Use the row height so the width does not depend on the delegate's layout geometry.
        const iconWidth = tasksRoot.height / Math.max(1, taskList.rows);
        return Math.min(maximum, Math.ceil(iconWidth + taskFrame.margins.left + taskFrame.margins.right
            + TaskManagerApplet.LayoutMetrics.labelMargin + textWidth + audioWidth));
    }
    Layout.maximumHeight: tasksRoot.vertical ? TaskManagerApplet.LayoutMetrics.preferredMaxHeight() + launcherSpacing : -1
    ...
    readonly property int launcherSpacing: !inPopup && !tasksRoot.iconsOnly && model.IsLauncher
        ? Plasmoid.configuration.launcherSpacing : 0
    ...
    onSmartLauncherEnabledChanged: {
        if (smartLauncherEnabled && !smartLauncherItem) {
            const component = Qt.createComponent("plasma.applet.org.kde.plasma.adjustabletaskmanager", "SmartLauncherItem");
